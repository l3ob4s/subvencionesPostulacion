...
use App\Models\Proyecto_tipoterritorio;
use App\Models\Proyecto_tipodifusion;
use App\Models\Proyecto_tipomediosverificacion;
...
use App\Models\Tipoterritorio;
use App\Models\Tipodifusion;
use App\Models\Tipomediosverificacion;
...
class Postulacionsubvenciones extends Component
{
    use WithFileUploads;
    ...
    //Vars Paso 0
    public $proyectos, $proyectosWithCodProyecto, $proyectosDD, $proyectosGralesNotDD, $proyectosMergeDDNotDD, $periodosPostu,
        $runOrganizacion, $dvRunOrganizacion, $flgDisabldBtnNvaPostulacion, $idIngreso;
    ...
    public $idOrganizacion, $rutRepLegal, $dvRutRepLegal, $nombreOrganizacion, $agnosExistencia, $codTipoVia, $nombreVia, $direccionOrganizacion,
        ...
        //Combobox Paso 1
        $bancos, $tipoCuentas, $tipoViasCmb, $tipoInstitucionCmb,
        //Rep Legal
    ...
    public $idProyecto, $codProyecto, $nombreProyecto, $nomEntidad, $tabActive, $idFondoConcursable, $montoProyecto, $duracionProyecto,
        ...
        // 27-08-2025 estos 3 campos reemplazaran a resumenProyecto MVillalobos solicitado por Loreto Assef (+)
        $descripProyectoPreg1, $descripProyectoPreg2, $descripProyectoPreg3, $descripTipoTerritorioOtro, $descripTipoDifusionOtro, $descripTipoMedioVerificacionOtro, $codTipoInstitucion,
        // 27-08-2025 MV(-)
    ...
    //Combobox Paso 2
    public $fondoConcursable, $provincias, $comunas, $tipoTerritorios, $tipoDifusion, $tipoMedioVerificacion, $codTipoTerritoriosCheck = [], $codTipoDifusionCheck = [], $codTipoMedioVerificacionCheck = [];
    ...
    public function inicio()
    {
        ...
        $this->tipoTerritorios = Tipoterritorio::all(); //27-08-2025
        $this->tipoDifusion = Tipodifusion::all(); //27-08-2025
        $this->tipoMedioVerificacion = Tipomediosverificacion::all(); //27-08-2025
        $this->provincias = Provincia::all();
        $this->bancos = Banco::all();
        $this->tipoCuentas = Tipocuenta::all();
        $this->tipoViasCmb = Tipovia::all(); //MV 20-03-2024 Solicitado por Joselyn San Juan
        $this->tipoInstitucionCmb = Tipoinstitucion::all(); //MV 28-08-2025 Solicitado por Faridi
    ...
    public function verificarCierrePostu()
    {
        ...
        $this->periodosPostu =  Periodospostu::whereRaw("DATE_FORMAT(fechaInicioPostu, '%Y-%m-%d') <= '" . $fechaActual . "' and DATE_FORMAT(fechaFinPostu, '%Y-%m-%d') >= '" . $fechaActual . "'")->first();

        if (empty($this->periodosPostu)) {
            $this->currentStep = -1;
        }
    }
...
//Prueba de reporte PDF Luego quitar codigo MV (+)
    ...
    //         ->where('proyectos.idProyecto', '=', 9503)->first();
    ...
    //     //29-08-2025 MVillalobos (+)
    //     $proy_TipoTerritorios = Proyecto_tipoterritorio::where('idProyectoFK', '=', $proyecto->idProyecto)->get();
    //     $proyTipoTerritorioStr = "";
    //       foreach($proy_TipoTerritorios as $item) {
    //         $tipoTerritorio = TipoTerritorio::where('codTipoTerritorio', '=', $item->codTipoTerritorio)->first();
    //         $proyTipoTerritorioStr .= $tipoTerritorio->descripcionTipoTerritorio;

    //         if ($item->codTipoTerritorio == 6) {//Otro 
    //             $proyTipoTerritorioStr .= ": ".$proyecto->descripTipoTerritorioOtro;
    //         } else {
    //             $proyTipoTerritorioStr .= ", ";
    //         }
    //       }

    //     $proy_TipoDifusion = Proyecto_tipodifusion::where('idProyectoFK', '=', $proyecto->idProyecto)->get();
    //     $proyTipoDifusionStr = "";
    //       foreach($proy_TipoDifusion as $item) {
    //         $tipoDifusion = TipoDifusion::where('codTipoDifusion', '=', $item->codTipoDifusion)->first();
    //         $proyTipoDifusionStr .= $tipoDifusion->descripcionTipoDifusion;

    //         if ($item->codTipoDifusion == 7) {//Otro 
    //             $proyTipoDifusionStr .= ": ".$proyecto->descripTipoDifusionOtro." ";
    //         } else {
    //             $proyTipoDifusionStr .= ", ";
    //         }
    //       }

    //     $proy_TipoMediosVerificacion = Proyecto_tipomediosverificacion::where('idProyectoFK', '=', $proyecto->idProyecto)->get();
    //     $proyTipoMedioVerificacionStr = "";
    //       foreach($proy_TipoMediosVerificacion as $item) {
    //         $tipoDifusion = TipoMediosVerificacion::where('codTipoMedioVerificacion', '=', $item->codTipoMedioVerificacion)->first();
    //         $proyTipoMedioVerificacionStr .= $tipoDifusion->descripcionTipoMedioVerificacion;

    //         if ($item->codTipoMedioVerificacion == 7) {//Otro 
    //             $proyTipoMedioVerificacionStr .= ": ".$proyecto->descripcionTipoMedioVerificacion." ";
    //         } else {
    //             $proyTipoMedioVerificacionStr .= ", ";
    //         }
    //       }
    ...
    //         'proyTipoTerritorioStr' => $proyTipoTerritorioStr, 
    //         'proyTipoDifusionStr' => $proyTipoDifusionStr,
    //         'proyTipoMedioVerificacionStr' => $proyTipoMedioVerificacionStr,
    //     ];

    //     $pdfResuPostu = PDF::loadView('resumenProyectPDF', $data)->output();
    ...
    //           $graphService->sendMail($data['asunto'], $bodyHtml, "mvillalobos@gorebiobio.cl", [$resProyectoAttach]); 
    //         //Envío de correo con MicrosoftGraphService MV 22-05-2025   
//Prueba de reporte PDF MV (-)
    ...
    {
        return view('livewire.postulacionsubvenciones');
    }
    ...
    public function cargarDatosProyectoSel($idProyecto)
    {
        try {
          ...
          //27-08-2025 MV (+)
            $this->descripProyectoPreg1 = $proyecto->descripProyectoPreg1;
            $this->descripProyectoPreg2 = $proyecto->descripProyectoPreg2;
            $this->descripProyectoPreg3 = $proyecto->descripProyectoPreg3;        
            $this->descripTipoTerritorioOtro = $proyecto->descripTipoTerritorioOtro; 
            $this->descripTipoDifusionOtro = $proyecto->descripTipoDifusionOtro;
             $this->descripTipoMedioVerificacionOtro = $proyecto->descripTipoMedioVerificacionOtro;
         
           $proyTipoTerritorio = Proyecto_tipoterritorio::where('idProyectoFK', '=', $proyecto->idProyecto)->get();
           foreach ($proyTipoTerritorio as $itemTerritorio) {
               $this->codTipoTerritoriosCheck[] = $itemTerritorio->codTipoTerritorio;
           }

           $proyTipoDifusion = Proyecto_tipodifusion::where('idProyectoFK', '=', $proyecto->idProyecto)->get();
            foreach ($proyTipoDifusion as $itemDifusion) {
              $this->codTipoDifusionCheck[] = $itemDifusion->codTipoDifusion;
            }

            $proyTipoMedioVerificacion = Proyecto_tipomediosverificacion::where('idProyectoFK', '=', $proyecto->idProyecto)->get();   
            foreach ($proyTipoMedioVerificacion as $itemMedioVerificacion) {
                $this->codTipoMedioVerificacionCheck[] = $itemMedioVerificacion->codTipoMedioVerificacion;
            }
          //27-08-2025 MV (-)
            ...
            $this->codTipoInstitucion = $organizacion->codTipoInstitucion; //28-08-2025 MV
            ...
            //MV 29-08-2025 (+)
            //1 = Tipo Institucion General, 2 = Municipios, 3 = ELEAM 
              $sql = "tipoproyectos.codTipoInstitucion = 1"; 
              if ($this->codTipoInstitucion == 1) { 
                    $sql = "tipoproyectos.codTipoInstitucion NOT IN (2, 3)";
              } else 
               if ($this->codTipoInstitucion == 2) {
                    $sql = "tipoproyectos.codTipoInstitucion != 3";
               } else 
                if ($this->codTipoInstitucion == 3) {
                    $sql = "tipoproyectos.codTipoInstitucion != 2";
                }
            //MV 29-08-2025 (-)
            ...
            //28-08-2025 Se oculta campo //30-08-2025 Se reincorpora Campo 
            $objetivoespecifico = Objetivosespecifico::where('idProyecto', '=', $proyecto->idProyecto)->get();
    ...
    public function updated($field, $value)
    {
        ...
        if ($this->currentStep == 0) {
           ...
           //Si las validaciones se estan realizando por tipo de proyecto y no por fondo concursable siempre se resetean (por ejemplo los tipos de proyectos correspondientes al fondo de asignacion directa) (+)
             if ($field == 'idTipoProyecto') {
                ...
                if (!empty($tipoProyecto)) {//Si existe al menos un monto maximo mayor a cero se realiza el reseteo de validacion por montos max y min asociados al fondo 
                     $this->resetValidation('montoProyecto');
                     $this->resetErrorBag('montoProyecto');
                     $this->resetValidation('duracionProyecto');
                     $this->resetErrorBag('duracionProyecto');     
                ...
                //30-08-2025 MVillalobos 
                  $this->resetValidation('idProvinciaProy');
                  $this->resetErrorBag('idProvinciaProy');
                 if ($this->idTipoProyecto != 116 && $this->idProvinciaProy == 4) { //Sólo el proyecto de impacto cultural idTipoProyecto = 116 tiene alcance regional 4 = Regional 
                    $this->addError('idProvinciaProy', 'El fondo seleccionado no tiene alcance regional, debe cambiar provincia y comuna para continuar.');      
                 }
             }
            //Si las validaciones se estan realizando por tipo de proyecto y no por fondo concursable siempre se resetean (por ejemplo los tipos de proyectos correspondientes al fondo de asignacion directa) (-)
        ...
        //MV 23-03-2025 Si existe un tipo de proyecto seleccionado y tiene un monto minimo y maximo asignado para validar, entonces se valida por tipo de proyecto
            ...
            //MV 23-03-2025  Si existe un tipo de proyecto seleccionado se verifica si tiene un monto minimo y maximo asignado para validar por tipo de proyecto 
                ...
                $this->validateOnly( 
                    ...
                    //28-08-2025 MV se quita validación de Objetivos especificos, se solicito ocultar el campo //30-08-2025 Se reincorpora el campo objetivos especificos 
            ...
            if ($field == 'idFondoConcursable') { 
                try {
                    ...
                    if ($this->idFondoConcursable > 0) {
                        // $this->validateFondoconcursable(); Se comenta puesto que la validacion se realiza al seleccionar el tipo de proyecto, puesto que Deportistas Destacados ahora es un tipo de proyecto y no un fondo concursable, al igual que ayudas tecnicas 01-09-2025 MV 
                ...
                //1 = Tipo Institucion General, 2 = Municipios, 3 = ELEAM 
                  $sql = "tipoproyectos.codTipoInstitucion = 1"; 
                  if ($this->codTipoInstitucion == 1) { //Si tipo de institucion es general no se muestran los tipos de proyectos asociados a municipios y ELEAM 
                        $sql = "tipoproyectos.codTipoInstitucion NOT IN (2, 3)";
                  } else 
                   if ($this->codTipoInstitucion == 2) {//Si tipo de institucion es Municipal no se muestran los tipos de proyectos asociados a ELEAM 
                        $sql = "tipoproyectos.codTipoInstitucion != 3";
                   } else 
                    if ($this->codTipoInstitucion == 3) {//Si tipo de institucion es ELEAM no se muestran los tipos de proyectos asociados a municipalidad 
                        $sql = "tipoproyectos.codTipoInstitucion != 2";
                    }
                //MV 29-08-2025 (-)
                  $tipoProyecto = Tipoproyecto::where('idFondoConcursable', '=', $this->idFondoConcursable)
        ...
        //MV 23-03-2025 Si al menos un tipo de proyecto asociado al fondo seleccionado tiene un monto maximo y minimo asignado se omite la validacion de maximo y minimo por el fondo (la validacion se realiza al seleccionar un tipo de proyecto) 
            ...
            if ($field == 'idTipoProyecto') {
                try {
                    //MV 23-03-2025 Si el tipo de proyecto es mayor a cero y tiene un monto asignado se considera el del proyecto y no el del fondo en el caso que los montos esten asociados al tipo de proyecto y no al fondo 
                        if ($this->idTipoProyecto > 0) { 
                            $this->validateFondoconcursable(); //La validacion se realiza al seleccionar el tipo de proyecto y no el fondo, puesto que Deportistas Destacados ahora es un tipo de proyecto y no un fondo concursable, al igual que ayudas tecnicas 01-09-2025 MV 
    ...
    public function cargarDatosRun()
    {
        ...
        if (!empty($organizacion)) {
            ...
            $this->dvRunOrganizacion = $organizacion->dvRunOrganizacion;
            $this->nombreOrganizacion = $organizacion->nombreOrganizacion;
            $this->codTipoInstitucion = $organizacion->codTipoInstitucion;//MV 28-08-2025 
            $this->agnosExistencia = $organizacion->agnosExistencia;
            $this->direccionOrganizacion = $organizacion->direccionOrganizacion;
            $this->codTipoVia = $organizacion->codTipoVia;
            $this->nombreVia = $organizacion->nombreVia;
        ...
        if ($this->idOrganizacion > 0) {
            ...
            $this->proyectos = Proyecto::join('fondoconcursables', 'fondoconcursables.idFondoConcursable', '=', 'proyectos.idFondoConcursableProyecto', 'left outer')
                ...
                ->where('proyectos.idFondoConcursableProyecto', '!=', 13) //MV 29-08-2025 Fondo distinto a asignacion directa  
                ->whereYear('proyectos.created_at', '=', now()->year)/*Todos los proyectos ingresado el años en curso*/
                ->WhereRaw("((proyectos.codProyecto is not null) or ((proyectos.codProyecto is null) and (proyectos.created_at >= '" . $this->periodosPostu->fechaInicioPostu . "' and proyectos.created_at <= '" . $this->periodosPostu->fechaFinPostu . "')))")
            ...
            if (!empty($this->proyectos)) {
                ...
                if (count($this->proyectos) > 0) {
                    ...
                    $this->dispatchBrowserEvent('swal:modal', [
                        ...
                        'mensaje' => '<span class="fs-5 text-primary" style="text-align:justify;">' . $msjProyStr . '</span>',
                        'id' => $idDiv,
                    ]);
                }
    ...
    public function getArrValidateDatosOrg()
    {
        return [
            ...
            'codTipoInstitucion' => 'required|gt:0',
            // 'agnosExistencia' => 'required|integer|digits_between:1,3|gt:-1',
            'agnosExistencia' => 'nullable|required_with:fecVencDirectiva|integer|digits_between:1,3|gte:2', //Este campo no es obligatorio para entidades publicas MV 04-09-2025 Faridi
    ...
    public function getArrValidateDatosRepLegal()
    {
        return [
            ...
            'fecVencDirectiva' => 'nullable|required_with:agnosExistencia|date_format:Y-m-d', //MV 28-08-2025 Se quita validacion de fecha obligatoria 
    ...
    public function firstStep()
    {
        ...
        if ($this->agnosExistencia == '') {
            $this->reset('agnosExistencia');
        }

         if ($this->fecVencDirectiva == '') {
            $this->reset('fecVencDirectiva');
        }
            ...
            try {
                ...
                $organizacion = Organizacion::updateOrCreate(
                    ...
                    [
                        ...
                        'codTipoInstitucion' => $this->codTipoInstitucion,//MV 28-08-2025 Faridi  
    ...
    public function getArrValidateDatosTecnicos()
    {
        return [
            ...
            //27-08-2025 'resumenProyecto' => 'required|string|max:1500',
            //27-08-2025
             'descripProyectoPreg1' => 'required|string|max:500',
             'descripProyectoPreg2' => 'required|string|max:500',
             'descripProyectoPreg3' => 'required|string|max:500',  
            //27-08-2025
            ...
            'codTipoTerritoriosCheck' => 'required|array|min:1',
            'codTipoDifusionCheck' => 'required|array|min:1',
            'codTipoMedioVerificacionCheck' => 'required|array|min:1',
...
//30-08-2025  
     public function validateFondoconcursable() {
        if ($this->idFondoConcursable != 13/*AD*/) {//No Tiene límite de postulaciones Ni montos minimos y máximos MV 17-05-2024 (Solicitado por Viviana Muñoz)
    //Se buscan todos los proyectos ingresados por la organizacion para el año en curso 
        $proyectos = Proyecto::join('fondoconcursables', 'fondoconcursables.idFondoConcursable', '=', 'proyectos.idFondoConcursableProyecto')
            ->where('idOrganizacionProyecto', '=', $this->idOrganizacion)
            ->where('proyectos.idFondoConcursableProyecto', '!=', 13) //MV 29-08-2025 Fondo distinto a asignacion directa  
        //Ver como hacer esta validacion correctamente    ->where('proyectos.idTipoProyecto', '!=', 107) //Tipo de proyecto = AYudas tecnicas no se contabiliza para el maximo de dos fondos en el año y solo puede postular una vez 31-08-2025 
        ...
        //   $tipoProyecto = Tipoproyecto::where('idTipoProyecto', '=', $this->idTipoProyecto)->first();
        //   $descripTipoProyecto =  ucwords(mb_strtolower($tipoProyecto->descripcionTipoProyecto, 'UTF-8')); 
 
        //31-08-2025 Una institución puede postular a dos tipos de Proyectos del fondo Deporte (DE) siempre y cuando uno de los proyectos sea de Deportista Destacado (109), en caso contrario solo puede postular a uno)

        if ($this->idTipoProyecto > 0) {
            if ($this->idFondoConcursable == 8) {//Fondo Deporte DE 
                 //Se obtienen todos los proyecto igual a deportistas destacados (idTipoProyecto = 109 (DEPORTISTAS DESTACADOS) Fondo: DEPORTISTAS DESTACADOS, Código: DE, idTipoFondo = 8, ingresados por la organizacion el año en curso
                  $proyectosDepDestacados = $proyectos->where('idTipoProyecto', '=', 109); //Tipo de proyecto = DEPORTISTAS DESTACADOS (idTipoProyecto = 109) no se contabiliza para el maximo de dos fondos en el año y puede postular 3 veces en el años 31-08-2025
                  $proyectosFondoDeportes = $proyectos->where('idFondoConcursableProyecto', '=', 8); //Tipo de proyecto = DEPORTISTAS DESTACADOS no se contabiliza para el maximo de dos fondos en el año y puede postular 3 veces en el años 31-08-2025
                  $proyectosNotFondoDeportes = $proyectos->where('idFondoConcursableProyecto', '!=', 8)->where('idTipoProyecto', '!=', 107); //Proyectos distintos al fondo deportes y distinto a ayudas tecnicas puesto que este tipo de proyecto es un comodin para las municipalidades 
                  $proyectosFondoDeportesNotDestacado = $proyectos->where('idFondoConcursableProyecto', '=', 8) ->where('idTipoProyecto', '!=', 109); //Todos los proyectos fondo deportes distintos a deportistas destacados




                 if (count($proyectosNotFondoDeportes) > 1) {//Si ya ha postulado a dos fondos distintos a Deporte en el año, no puede postular al fondo Deporte aunque postule a Deportista Destacado 
                    //Si existen mas de un proyecto no se puede postular 
                     throw new Exception('<span class="text-primary">Máximo puede postular a dos fondos distintos durante el año. Exceptuando los tipos de proyecto <b class="text-success">"Ayuda Técnica(Sólo Municipios)" del Fondo "Discapacidad e inclusión" 1 vez al año, independientemente que ya haya ingresado el máximo de dos postulaciones</b>.</span>');
                 }

                //Si desea postular a un proyecto de deportista destacado y ya tiene un proyecto ingresado de deportista destacado no puede postular nuevamente 
                 if ($this->idTipoProyecto == 109 && count($proyectosDepDestacados) > 0) {
                    throw new Exception('<span class="text-primary">Usted ya ha ingresado un proyecto del tipo <b class="text-success">"Deportista destacado"</b>.</span>');
                 }  

              
                
                 if (count($proyectosFondoDeportesNotDestacado) > 0 && $this->idTipoProyecto != 109) { //Si ya ha postulado a un proyecto del fondo deportes distinto a deportista destacado no puede postular a otro proyecto del mismo fondo que sea distinto a deportista destacado
                    throw new Exception('<span class="text-primary">Usted ya ha postulado a un proyecto correspondiente al fondo <b class="text-success">"Deporte"</b>. Podrá postular a dos proyectos de este fondo en el año solamente en el caso que uno de ellos corresponda a <b class="text-success">Deportistas Destacados</b>.</span>');
                 }
                 


                // if ($this->idTipoProyecto != 109 && count($proyectosFondoDeportes) > 0) { //Si Tipo de Proyecto es disinto a Deportista Destacado y ya ha postulado a un proyecto correspondiente al fondo deportes no puede postular a otro proyecto del mismo fondo
                //   //Maximo puede postular 1 vez en el año al tipo de proyecto deportista destacado                      
                //   throw new Exception('<span class="text-primary">Usted ya ha postulado a un proyecto correspondiente al fondo <b class="text-success">"Deporte"</b>. Podrá postular a dos proyectos de este fondo en el año solamente en el caso que uno de ellos corresponda a <b class="text-success">Deportistas Destacados</b>.</span>');
                // }
              
            //Si desea postular a un proyecto de deportista destacado y ya ha ingresado el maximo de dos postulaciones en el año no puede postular al fondo  
            //(Solo puede postular a dos fondos en el año si uno de los proyectos corresponde a Deportistas Destacados)
                if ($this->idTipoProyecto == 109 && count($proyectosFondoDeportes) > 1) { 
                   throw new Exception('Usted ya ha ingresado el máximo de postulaciones correspondientes al fondo <b class="text-success">"Deporte"</b>.');
                }  

            //Una vez validado lo anterior se verifica si ya ha postulado al maximo de dos fondos distintos durante el año 
                // if (count($proyectos) > 1) {
                  
                //     //Si existe mas de un proyecto no se puede postular 
                //     throw new Exception('<span class="text-primary">Máximo puede postular a dos fondos distintos durante el año. Exceptuando el tipo de proyecto <b class="text-success">"Ayuda Técnica(Sólo Municipios)" del Fondo "Discapacidad e inclusión" 1 vez al año</b>. Independientemente que ya haya ingresado el máximo de dos postulaciones</span>');
                // }            
            }
            else  
              if ($this->idFondoConcursable == 19 && $this->idTipoProyecto == 107) { //Fondo Discapacidad e Inclusión DI, Tipo Proyecto = Ayudas Técnicas 
                //Se obtienen todos los proyecto igual a ayudas tecnicas (idTipoProyecto = 107 (AYUDAS TÉCNICAS (Sólo Municipios)) Fondo: DISCAPACIDA E INCLUSION, Código: DI, idTipoFondo = 19, ingresados por la organizacion el año en curso
                 $proyectosAyudasTecnicas = $proyectos->where('idTipoProyecto', '=', 107); //Tipo de proyecto = AYUDAS TECNICAS no se contabiliza para el maximo de dos fondos en el año y solo puede postular una vez las municipalidades 31-08-2025  
                //Maximo puede postular 1 vez en el año al tipo de proyecto ayudas técnicas 
                if (count($proyectosAyudasTecnicas) > 0) {
                    throw new Exception("Ya ha postulado un proyecto de ayudas técnicas correspondiente al fondo de <span class='text-success fw-bold'>Discapacidad e Inclusión</span>, máximo puede postular una vez al año.");
                }
              } else
                 if ($this->idFondoConcursable == 9) { //Validacion Fondo Cultura CU  12-09-2025 
                      //Se obtienen todos los proyecto igual a cultura (idFondoConcursable = 9 (CULTURA) ingresados por la organizacion el año en curso
                          $proyectosFondoCultura = $proyectos->where('idFondoConcursableProyecto', '=', 9); //Fondo = CULTURA las instituciones pueden postular a dos tipos de proyectos del fondo Cultura en el año 12-09-2025  
                          $proyectosNotFondoCultura = $proyectos->where('idFondoConcursableProyecto', '!=', 9)->where('idTipoProyecto', '!=', 107); //Proyectos distintos al fondo cultura y distinto a ayudas tecnicas puesto que este tipo de proyecto es un comodin para las municipalidades  

                         if (count($proyectosNotFondoCultura) > 1) {//Si ya ha postulado a dos fondos distintos a Cultura en el año, no puede postular a un tercer fondo aunque postule a Cultura  
                            //Si existen mas de un proyecto no se puede postular 
                             throw new Exception('<span class="text-primary">Máximo puede postular a dos fondos distintos durante el año. Exceptuando los tipos de proyecto <b class="text-success">"Ayuda Técnica(Sólo Municipios)" del Fondo "Discapacidad e inclusión" 1 vez al año, independientemente que ya haya ingresado el máximo de dos postulaciones</b>.</span>');
                         }

                         if (count($proyectosFondoCultura) > 1) {//Si ya ha postulado al maximo de dos proyectos del fondo a Cultura en el año, se le envia mensaje
                             throw new Exception('<span class="text-primary">Máximo puede postular a dos proyectos del fondo cultura en el año.</span>');
                         }

                         $proyectosFondoCulturaTipoProyectoSel = $proyectosFondoCultura->where('idTipoProyecto', '=', $this->idTipoProyecto); //Tipo de proyecto del fondo cultura al que desea postular
                         $tipoProyectoPaso = Tipoproyecto::where('idTipoProyecto', '=', $this->idTipoProyecto)->first();

                         if (count($proyectosFondoCulturaTipoProyectoSel) > 0) { //Si ya ha postulado a un proyecto correspondiente al tipo de proyecto seleccionado no puede postular nuevamente 
                           throw new Exception('<span class="text-primary">Usted ya ha postulado al tipo de proyecto <b class="text-success">"'.$tipoProyectoPaso->descripcionTipoProyecto.'"</b> correspondiente al fondo <b class="text-success">"Cultura"</b>.</span>');
                         }
                }
               else 
                if ($this->codTipoInstitucion == 2/*Municipalidad*/) {        
                    //Para las municipalidades no se contabiliza el proyecto de ayudas técnicas para el maximo de dos fondos en el año y solo puede postular una vez las municipalidades
                    $proyectosNotAyudaTecnicaDI = $proyectos->where('idTipoProyecto', '!=', 107); //Se consulta por los proyectos distintos a AYUDAS TECNICAS del fondo DI no se contabiliza para el maximo de dos fondos en el año y solo puede postular una vez las municipalidades 

                    if (count($proyectosNotAyudaTecnicaDI) > 2) {
                         throw new Exception('<span class="text-primary">Ha postulado al máximo de proyectos para el año en curso.</span>');
                    }                      
                } else
                    if (count($proyectos) > 1) {
                        //Se omiten los Tipos de proyectos: 109 = Deportistas destacados Fondo: DE, 107 = Ayudas técnicas Fondo: DI en la contabilización de postulaciones (son comodines, las demas validaciones se realizan mas arriba)
                        $proyectoPaso = $proyectos->where('idTipoProyecto', '!=', 109)->where('idTipoProyecto', '!=', 107); 

                         if(count($proyectoPaso) > 1) {
                          //Si existen mas de un proyecto no se puede postular 
                           throw new Exception('<span class="text-primary">Máximo puede postular a dos fondos distintos durante el año. Exceptuando los tipos de proyecto <b class="text-success">"Ayuda Técnica(Sólo Municipios)" del Fondo "Discapacidad e inclusión" 1 vez al año. Independientemente que ya haya ingresado el máximo de dos postulaciones.</b> En cuanto al fondo <b class="text-success">"Deporte"</b> podrá postular a dos proyectos en el año, sólo si uno de ellos corresponde a <b class="text-success">Deportistas Destacados</b>.</span>');
                         }
                    }
                 else {                   
                     //Se obtienen todos los proyectos distintos al proyecto que se está editando
                        $proyectosPaso = $proyectos->where('idProyecto', '!=', $this->idProyecto);
                
                        if (count($proyectosPaso->where('idFondoConcursableProyecto', '=', $this->idFondoConcursable)) > 0) {
                          //Ya existe un proyecto para el fondo seleccionado no puede postular al mismo fondo en un año 
                            throw new Exception('<span class="text-primary">Ya ha postulado un proyecto para el fondo <span class="text-success fw-bold">' . $this->descripcionFondo . '</span>, Máximo puede postular a dos fondos distintos durante el año. Exceptuando los tipos de proyecto <b class="text-success">"Ayuda Técnica(Sólo Municipios)" del Fondo "Discapacidad e inclusión" 1 vez al año</b>, independientemente que ya haya ingresado el máximo de dos postulaciones. En cuanto al fondo <b class="text-success">"Deporte"</b> podrá postular a dos proyectos en el año, sólo si uno de ellos corresponde a <b class="text-success">Deportistas Destacados</b>.</span>');
                        }
                 }
        }
...
//Se comenta funcion validateFondoconcursable 30-08-2025
    public function validateFondoconcursableAnterior() {
        // try { 
   ...
   if ($this->idFondoConcursable != 13/*AD*/) {//Tiene límite de postulaciones MV 17-05-2024 (Solicitado por Viviana Muñoz)
        //Modo edicion Proyecto
        if ($this->idProyecto > 0) {
            /*Si el fondo seleccionado es igual a Deportista destacado*/
            if ($this->idFondoConcursable == 1) {
                //Se obtienen todos los proyectos Deportista Destacado distintos al proyecto que se está editando
                $proyectosDDPaso = $proyectosDD->where('idProyecto', '!=', $this->idProyecto);
                //Si cantidad de proyectos ingresados igual a Deportista destacado es mayor a dos no puede crear uno nuevo
                //Solo puede postular a dos proyectos de fondos correspondientes a deportista destacado en el año
                //Si fondo concursable seleccionado es igual a DD y la cantidad de proyectos DD es mayor a 1
                if (count($proyectosDDPaso) > 2) {
                    throw new Exception("Ya ha postulado al máximo de 3 veces en el año al fondo deportista destacado");
                }
            } else
                /*Si el fondo seleccionado es igual a artistas regionales*/
                if ($this->idFondoConcursable == 11) {
                    //Se obtienen todos los proyectos artistas regionales distintos al proyecto que se está editando
                    $proyectosARPaso = $proyectosAR->where('idProyecto', '!=', $this->idProyecto);
                    //Si cantidad de proyectos ingresados igual a artistas regionales es mayor a dos no puede crear uno nuevo
                    //Solo puede postular a tres proyectos de fondos correspondientes a artistas regionales en el año

                    //Si fondo concursable seleccionado es igual a AR y la cantidad de proyectos AR es mayor a 1
                    if (count($proyectosARPaso) > 2) {
                        throw new Exception("Ya ha postulado al máximo de 3 veces en el año al fondo artistas regionales");
                    }
                } else
                    /*Si el fondo seleccionado es igual a impacto cultural*/
                    if ($this->idFondoConcursable == 12) {
                        //Se obtienen todos los proyectos impacto cultural distintos al proyecto que se está editando
                        $proyectosICPaso = $proyectosIC->where('idProyecto', '!=', $this->idProyecto);
                        //Si cantidad de proyectos ingresados igual a impacto cultural es mayor a dos no puede crear uno nuevo
                        //Solo puede postular a 2 proyectos de fondos correspondientes a impacto cultural en el año

                        //Si fondo concursable seleccionado es igual a IC y la cantidad de proyectos IC es mayor a 2
                        if (count($proyectosICPaso) > 2) {
                              throw new Exception("Ya ha postulado al máximo de 3 veces en el año al fondo impacto cultural");
                        }
                    }
                    // /*Si el fondo seleccionado es igual a asignacion directa*/
                    //     if ($this->idFondoConcursable == 13) {
                    //         //Se obtienen todos los proyectos asignacion directa distintos al proyecto que se está editando
                    //         $proyectosADPaso = $proyectosAD->where('idProyecto', '!=', $this->idProyecto);
                    //         //Si cantidad de proyectos ingresados igual a asignación directa es mayor a 0 no puede crear uno nuevo (maximo una postulación al año independiente de los otros fondos)
                    //         //Solo puede postular a 1 proyecto de fondos correspondientes a asignacion directa en el año
    
                    //         //Si fondo concursable seleccionado es igual a AD y la cantidad de proyectos IC es mayor a 0
                    //         if (count($proyectosADPaso) > 0) {
                    //               throw new Exception("Ya postuló a un proyecto correspondiente al fondo Asignación Directa. Máximo puede postular un proyecto al año correspondiente a ese fondo.");
                    //         }
                    //     }
                    else {  //Si fondo seleccionado es distinto a Deportista Destacado, Artistas regionales, Impacto cultural y Asignación directa 
                        //Se obtienen todos los proyectos distintos al proyecto que se está editando
                        $proyectosNotDDARPaso = $proyectosNotDDAR->where('idProyecto', '!=', $this->idProyecto);

                        //Si el fondo concursable seleccionado se encuentra en los demas proyectos enviar un mensaje
                        if (count($proyectosNotDDARPaso->where('idFondoConcursableProyecto', $this->idFondoConcursable)) > 0) {
                            //Ya existe un proyecto para el fondo seleccionado no puede postular al mismo fondo en un año
                            throw new Exception('Ya ha postulado un proyecto para el fondo <span class="text-success fw-bold">' . $this->descripcionFondo . '</span>, máximo puede postular a un proyecto durante el año. Exceptuando los proyectos relacionados con los fondos: deportistas destacados e impacto cultural, a lo cuáles puede postular hasta 3 veces en el año');
                        }
                    }
        } else { //Modo insert Proyecto
            //Si idProyecto == 0 se está creando un nuevo proyecto por lo tanto la cantidad de proyectos creados distintos a DD debe ser igual a 1 para poder crear uno nuevo
            //Si fondo seleccionado es igual a deportista destacado
            if ($this->idFondoConcursable == 1) {
                //Si fondo concursable seleccionado es igual a DD y la cantidad de proyectos DD es igual a 3
                if (count($proyectosDD) > 2) {
                    throw new Exception("Ya ha postulado al máximo de 3 veces en el año al fondo deportista destacados.");
                }
            } else
                //Si fondo seleccionado es igual a artistas regionales
                if ($this->idFondoConcursable == 11) {
                    //Si fondo concursable seleccionado es igual a AR y la cantidad de proyectos AR es igual a 3
                    if (count($proyectosAR) > 2) {
                        throw new Exception("Ya ha postulado al máximo de 3 veces en el año al fondo artistas regionales.");
                    }
                }
                else
                if ($this->idFondoConcursable == 12) {
                    //Si fondo concursable seleccionado es igual a IC y la cantidad de proyectos IC es igual a 3
                    if (count($proyectosAR) > 2) {
                        throw new Exception("Ya ha postulado al máximo de 3 veces en el año al fondo impacto cultural.");
                    }
                }
                if ($this->idFondoConcursable == 16) {//29/07/2024 
                    //Si fondo concursable seleccionado es igual a AC y la cantidad de proyectos AC es igual a 1
                    if (count($proyectosAC) > 0) {
                        throw new Exception("Ya ha postulado al máximo de 1 vez al año al fondo actividades culturales masivas.");
                    }
                }
                else //Sino si fondo seleccionado es distinto a deportista destacado y artistas regionales
                if (!empty($proyectosNotDDAR)) {
                    //Si cantidad de proyectos ingresados distintos a Deportista destacado y artistas regionales es mayor a 1 no se puede crear uno nuevo (insert)
                    if (count($proyectosNotDDAR) > 0) {
                       //Solo puede postular a dos proyectos distintos a DD y AR en el año comentado por MV 19-04-2024
                       // throw new Exception("Ya ha postulado el máximo de 2 veces al año a proyectos distintos a deportista a destacado");

                       //Solo puede postular a 1 proyecto distinto a: DD, AR e IC en el año   MV 19-04-2024
                        throw new Exception("Máximo puede postular 1 vez al año a proyectos distintos a: deportistas destacados.");
                    }

                    //Si el fondo concursable seleccionado se encuentra en los demas proyectos enviar un mensaje
                    if (count($proyectosNotDDAR->where('idFondoConcursableProyecto', $this->idFondoConcursable)) > 0) {
                        throw new Exception('Ya has postulado un proyecto para el fondo <span class="text-success fw-bold">' . $this->descripcionFondo . '</span>, máximo puedes postular a dos fondos distintos durante el año'); //. Exceptuando los fondos destinados a deportistas destacados y artistas regionales a los cuáles puede postular 3 veces en el año');
                    }
                }
        }
       }//Fin validación AD
    ...
    public function secondStep()
    {
        $this->withValidator(function (Validator $validator) {
            ...
            //28-08-2025 MV se quita validación de Objetivos especificos, se solicito ocultar el campo //30-08-2025 Se reincorporan los objetivos especificos 
       ...
       if ($this->idTipoProyecto != 116 && $this->idProvinciaProy == 4) {
               $this->addError('idProvinciaProy', 'El fondo seleccionado no tiene alcance regional, debe cambiar provincia y comuna para continuar.');   
               $this->dispatchBrowserEvent('moveScrollByErrorId', ['id' => '#ididProvinciaProy']);  
            return;        
        }

        //Ver como validar los meses de ejecucion en las actividades de acuerdo a la duracion maxima del proyecto
        // idmesesEjecuActividad1Error, idmesesEjecuActividad1Error
        // foreach ($this->inputsActividad as $index => $itemActividad) { 
        //     $mesesEjecuArr = $itemActividad['mesesEjecuActividad'];
        //     foreach ($mesesEjecuArr as $index => $itemMeses) {
        //          $this->dispatchBrowserEvent('moveScrollByErrorId', ['id' => '#idmesesEjecuActividad'. $index]); //Mover Scroll al campo con el error
        //         //  dd($itemMeses);
            ...
            try {
                ...
                $proyecto = Proyecto::updateOrCreate(
                    ...
                    [
                        ...
                        'descripProyectoPreg1' => $this->descripProyectoPreg1, //27-08-2025 MV
                        'descripProyectoPreg2' => $this->descripProyectoPreg2, //27-08-2025 MV
                        'descripProyectoPreg3' => $this->descripProyectoPreg3, //27-08-2025 MV
                        ...
                        //28-08-2025 MVillalobos
                        'descripTipoTerritorioOtro' => $this->descripTipoTerritorioOtro, 
                        'descripTipoDifusionOtro' => $this->descripTipoDifusionOtro, 
                        'descripTipoMedioVerificacionOtro' => $this->descripTipoMedioVerificacionOtro,
                    ]
                );
          
                Proyecto_tipoterritorio::where('idProyectoFK', '=', $proyecto->idProyecto)->delete();
                foreach ($this->codTipoTerritoriosCheck as $itemTerritorio) { 
                        Proyecto_tipoterritorio::updateOrCreate( ['idProyectoFK' => $proyecto->idProyecto, 'codTipoTerritorio' => $itemTerritorio], 
                         [
                            'idProyectoFK' => $proyecto->idProyecto,
                            'codTipoTerritorio' => $itemTerritorio,
                         ]);
                }

                Proyecto_tipodifusion::where('idProyectoFK', '=', $proyecto->idProyecto)->delete();
                foreach ($this->codTipoDifusionCheck as $itemdifusion) {   
                        Proyecto_tipodifusion::updateOrCreate( ['idProyectoFK' => $proyecto->idProyecto, 'codTipoDifusion' => $itemdifusion], 
                            [
                                'idProyectoFK' => $proyecto->idProyecto,
                                'codTipoDifusion' => $itemdifusion,
                            ]);
                } 

            Proyecto_tipomediosverificacion::where('idProyectoFK', '=', $proyecto->idProyecto)->delete(); 
                foreach ($this->codTipoMedioVerificacionCheck as $itemMedVerificacion) {  
                        Proyecto_tipomediosverificacion::updateOrCreate( ['idProyectoFK' => $proyecto->idProyecto, 'codTipoMedioVerificacion' => $itemMedVerificacion], 
                            [
                            'idProyectoFK' => $proyecto->idProyecto,
                            'codTipoMedioVerificacion' => $itemMedVerificacion,
                            ]);
                } 

            //MVillalobos 28-08-2025 se quita los objetivos especificos, puesto que se solicito ocultar el campo (+)//Se reincorporan los objetivos especificos 30-08-2025 
        ...
        try {
            ...
            if (count($this->inputsRecursosHumanos) == 0 && count($this->inputsGastosGenerales) == 0 && count($this->inputsEquipamiento) == 0) {
                ...
                if (count($this->inputsEquipamiento) > 0) {
                    ...
                    //30-08-2025 se traera desde la BD $maxPorcEquip = $this->idFondoConcursable == 6/*PM*/ ? 60 : 80; //Para personas mayores el porcentaje en equipamiento es de 60%, para los otros fondos es de 80%

                    //MV 30-08-2025 Los porcentajes se obtienen desde las tablas fondoconcursables o tipoProyecto 
                    $fondoConcursablePaso = $this->fondoConcursable->where('idFondoConcursable', '=', $this->idFondoConcursable)->first();
                    $maxPorcEquip = $fondoConcursablePaso->porcEquipamiento;
                    $mensajeValidPorcentaje = " para el fondo: ".ucwords(mb_strtolower($fondoConcursablePaso->descripcionFondoConcursable, 'UTF-8'));  

                    //MV 23-05-2025 Si el tipo de proyecto y tiene un monto asignado se considera el del proyecto y no el del fondo
                    if ($this->idTipoProyecto > 0) {
                        $tipoProyecto = Tipoproyecto::where('idTipoProyecto', '=', $this->idTipoProyecto)->first();
                        if (!empty($tipoProyecto->porcEquipamiento)) {
                        if ($tipoProyecto->porcEquipamiento > 0)  {
                            $maxPorcEquip = $tipoProyecto->porcEquipamiento;                             
                        }
                        $mensajeValidPorcentaje = " para el tipo de proyecto: ".ucwords(mb_strtolower($tipoProyecto->descripcionTipoProyecto, 'UTF-8')); 
                       }
                    } 

                    if ($porcentajeEquip > $maxPorcEquip) {
                        throw new Exception('<span class="text-success">El total en equipamiento ' . $mensajeValidPorcentaje. ' no puede ser mayor al ' . $maxPorcEquip . '% (tope máximo: $' . number_format($this->montoProyecto * ($maxPorcEquip / 100), 0, ',', '.') . ') del monto total a financiar: $' . number_format($this->montoProyecto, 0, ',', '.') . '</span>');
                    } else
                        //Si ingresa un item en equipamiento debe ingresar un item en RRHH o GG para completar el porcentaje restante 
    ...
    public function enviarCorreoPostulacion()  
    {
        ...
        $proyTipoMedioVerificacionStr = "";
          foreach($proy_TipoMediosVerificacion as $item) {
            ...
            if ($item->codTipoMedioVerificacion == 7) {//Otro 
                $proyTipoMedioVerificacionStr .= ": ".$proyecto->descripTipoMedioVerificacionOtro." "; 
            } else {
                $proyTipoMedioVerificacionStr .= ", ";
            }
          }
